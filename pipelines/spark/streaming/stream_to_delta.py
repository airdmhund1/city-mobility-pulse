...
def main():
    ...
    df_bike_raw = read_kafka_stream(spark, TOPIC_BIKE).select(
    ...
    q_bronze_bike = write_delta(df_bike_raw, BRONZE_BIKE, CKPT_BRONZE_BIKE, partition_cols=["dt"])

    df_weather_raw = read_kafka_stream(spark, TOPIC_WEATHER).select(
        col("key").cast("string").alias("key"),
        col("value").cast("string").alias("value"),
        col("topic"), col("partition"), col("offset"),
        col("timestamp").alias("kafka_timestamp"),
    ...
    q_bronze_weather = write_delta(df_weather_raw, BRONZE_WEATHER, CKPT_BRONZE_WEATHER, partition_cols=["dt"])
    ...
    df_bike_silver = (
        df_bike_raw
        .select(from_json(col("value"), bike_schema).alias("j"))
        ...
        .withColumn(
            "availability_rate",
            expr("CASE WHEN docks_total IS NOT NULL AND docks_total > 0 THEN bikes_available / docks_total ELSE NULL END")
        )
        .withColumn(
            "utilization_rate",
            expr("CASE WHEN docks_total IS NOT NULL AND docks_total > 0 THEN bikes_occupied / docks_total ELSE NULL END")
        )
    )

    q_silver_bike = (
        df_bike_silver.writeStream
            .format("delta")
    ...
    df_weather_silver = (
        df_weather_raw
        .select(from_json(col("value"), weather_schema).alias("j"))
